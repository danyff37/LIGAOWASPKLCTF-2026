...
* **Objective:** Extract a hidden secret message from a Linux ELF binary by bypassing its intended unlock mechanism and reversing its custom encryption.
...
* `cat` / `strings` - For initial static analysis
* CyberChef or Python - For applying ROT13 and string reversal

---

## Step-by-Step Solution

### Step 1: Initial Analysis & Identifying the Obfuscation

The challenge provides two files: a compiled binary (`lockbox`) and a text file (`friend_note.txt`). According to the note, the binary requires a 64-character HMAC key passed via an `--unlock` argument to decrypt a secret message. The author brags about using three layers of protection: scattering the data across memory, reversing the string, and applying ROT13 encoding.
...
The author claims that running `strings` on the binary yields "nothing useful." However, looking directly at the raw ASCII output of the binary reveals artifacts of the program's hardcoded execution instructions. In C/C++, when a string is hardcoded and constructed at runtime, the compiler often breaks it into 8-byte (64-bit) chunks and moves them directly onto the stack using `mov` instructions. By inspecting the raw dump of `lockbox`, we can spot these exact stack operations mixed with ASCII characters:
